...
## 2. Pre-Processing
 - 조졌다. 이미지들이 모두 FHD급 이미지들임. 먼저 resolution을 떨구고 이미지 태깅을 맡겼어야 했는데 FHD급 그대로 xml파일이 다 만들어졌음. 혹시 이 상태에서도 이미지 resolution을 떨어뜨리면 xml에 태깅된 박스 정보들도 같은 비율로 줄이는 방법이 있나..? 있을거 같긴한데 하기 싫다. 일단 그냥 하자.
...
## 4. Creating TFRecords
 - 용량이 250GB 정도의 상당한 양이므로 이미지 100개씩만 뽑아서 변환을 테스트해야한다.
 - 이 과정이 이제 핵심인데 변환 후 TFrecord를 시각화 시켜주는 툴인 tfviewer 를 사용하여 확인해본다. (이와 관련해서 웹브라우저로 볼 수 있게 링크를 추가할 예정)
 - 학습시킬 객체 이외에 xml에 태깅된 정보들이 있을 것이다. 그러니깐 label_map에 없는 클래스가 xml파일에 있어도 되는 것인가? 학습에 영향을 주나?
