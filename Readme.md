...
## 2. Pre-Processing
 ...
 - 데이터 축소시키며 박스도 줄일 것임.
 => 이와 관련하여 전체 데이터셋을 300*300 으로 축소시켰다. 
 
 - 데이터셋 전처리가 제일 중요하다. 학습시킬 클래스들만 남겨놓고 나머지 객체들에 대한 태그들은 지우는 것 또한 좋다. 왜냐하면 tfrecord로 데이터를 serialize 시킬 때 불필요한 태그가 껴있다면 제대로 처리가 안 된다.
...
## 4. Creating TFRecords
 ...
 - 학습시킬 객체 이외에 xml에 태깅된 정보들이 있을 것이다. 그러니깐 label_map에 없는 클래스가 xml파일에 있어도 되는 것인가? 학습에 영향을 주나? 정답은 그렇다 이다. 왜냐하면 tfrecord로 만드는 스크립트에서 예외처리를 제대로 못 시킨다면 문제가 생긴다. 그러니 전처리 과정에서 불필요한 태그와 같은 데이터는 제거하는 편이 속 편하다.
