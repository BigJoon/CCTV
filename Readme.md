...
## 2. Pre-Processing
 - 조졌다. 이미지들이 모두 FHD급 이미지들임. 먼저 resolution을 떨구고 이미지 태깅을 맡겼어야 했는데 FHD급 그대로 xml파일이 다 만들어졌음. 혹시 이 상태에서도 이미지 resolution을 떨어뜨리면 xml에 태깅된 박스 정보들도 같은 비율로 줄이는 방법이 있나..? 있을거 같긴한데 하기 싫다. 일단 그냥 하자. =>이와 관련하여 issue 에 기록한 것이 해결책이 될 수도 있다. 
